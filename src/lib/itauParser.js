    ...
    try {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            if (!dueDate) {
                ...
                for (let k = 0; k < textItems.length; k++) {
                    if (textItems[k].includes('Vencimento:') && textItems[k+1] && /^\d{2}\/\d{2}\/\d{4}$/.test(textItems[k+1])) {
                    ...
                    const match = textItems[k].match(/Vencimento:\s*(\d{2}\/\d{2}\/\d{4})/);
                    if (match) {
                ...
                if (!dueDate) {
                    dueDateMatch = fullText.match(/Vencimento:\s*(\d{2}\/\d{2}\/\d{4})/i) || fullText.match(/(\d{2}\/\d{2}\/\d{4})\s*R\$/);
                    if (dueDateMatch) {
            ...
            for (let j = 0; j < textItems.length; j++) {
                ...
                // Detect Card Section Header: "ANDRE DIAS DA SILVA (final 3290)"
                // Based on OCR: "ANDRE DIAS DA SILVA (final 3290)"
                ...
                if (/^\d{2}\/\d{2}$/.test(item)) {
                    ...
                    while (k < j + 15 && k < textItems.length) { // Increased window
                        ...
                        if (/^\d{2}\/\d{2}$/.test(candidate) || candidate.includes('Lançamentos')) {
                            break;
                        }
                    ...
                    if (amountStr && descriptionParts.length > 0) {
                        ...
                        if (!isNaN(amount)) {
                            ...
                            if (!description.toUpperCase().includes('TOTAL') && !description.toUpperCase().includes('SALDO')) {
                                transactions.push({
                                    date: finalDate,
        ...
        const uniqueTransactions = transactions.filter((t, index, self) =>
            index === self.findIndex((t2) => (
                t2.date === t.date && t2.description === t.description && t2.amount === t.amount
            ))
        );
